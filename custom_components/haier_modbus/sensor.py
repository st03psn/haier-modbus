...
class HaierPvStatus(HaierModbusEntity, SensorEntity):
    ...
    Zeigt bei jedem Poll, was ``pv.py`` gerade tut — die WP-Zyklus-Stufe (Basis /
    Erhöht) und ob Boost (WP + Heizstab gemeinsam) läuft. Als Attribute: aktueller
    Überschuss, effektiver Sollwert, „WP läuft", „Heizstab an". Im Modus Aus/Executor:
    ``off``.
    """
